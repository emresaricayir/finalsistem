...
<div id="top-header" class="bg-white shadow-sm min-h-[88px] flex items-center">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex items-center justify-between relative w-full">
        <a href="{{ route('welcome') }}" class="flex items-center space-x-4 -ml-2 hover:opacity-80 transition-opacity duration-200">
            ...
            <div>
                @if(!empty($orgSubtitle))
                    <div class="text-[10px] md:text-xs text-red-600 tracking-widest">{{ $orgSubtitle }}</div>
                @endif
                <div class="text-sm md:text-lg font-bold text-gray-900">{{ $orgName }}</div>
            </div>
        </a>
