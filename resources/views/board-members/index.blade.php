...
<head>
    ...
    <title>Yönetim Kurulu - {{ $orgName }}</title>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                <div class="flex items-center justify-between">
                    ...
                    <nav class="flex items-center" aria-label="Breadcrumb">
                        ...
                        <span class="text-sm font-semibold text-teal-800">Yönetim Kurulu</span>
                    </nav>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div class="hidden lg:block lg:col-span-1">
                    <div class="elegant-card rounded-xl p-6 sticky-sidebar">
                        <div class="mb-8">
                            <h2 class="text-xl font-bold text-gray-900 mb-2">Hızlı Menü</h2>
                ...
                <!-- Right Content - Yönetim Kurulu -->
                <div class="lg:col-span-3">
                    <div class="elegant-card rounded-xl p-8">
                        <div class="mb-8">
                            <h1 class="text-3xl font-bold text-teal-800 mb-4">Yönetim Kurulu</h1>
                            <div class="elegant-divider"></div>
                        </div>

                        @if($boardMembers->count() > 0)
                            @php
                                $president = $boardMembers->where('title', 'Başkan')->first();
                                $otherMembers = $boardMembers->where('title', '!=', 'Başkan');
                            @endphp

                            <!-- President Section -->
                            @if($president)
                                <div class="mb-12">
                                    <h2 class="section-title text-2xl font-bold text-gray-900 mb-8 text-center">Başkan</h2>
                                    <div class="flex justify-center">
                                        <div class="member-card bg-white rounded-xl shadow-md overflow-hidden w-80 fade-in">
                                            ...
                                            <div class="member-info">
                                                <h3 class="text-lg font-bold text-gray-900 mb-2">{{ $president->name }}</h3>
                                                <p class="text-teal-600 font-semibold mb-3 text-sm">{{ $president->title }}</p>

                                                @if(!empty($president->bio))
                                                    <p class="text-gray-600 text-sm leading-relaxed mb-4">{{ Str::limit($president->bio, 120) }}</p>
                                                @endif

                                                <!-- Contact & Social -->
                                                <div class="flex flex-wrap justify-center gap-2">
                                                    @if($president->email)
                                                        <a href="mailto:{{ $president->email }}" class="contact-btn w-8 h-8 bg-teal-100 text-teal-600 rounded-full flex items-center justify-center hover:bg-teal-200" title="E-posta">📧</a>
                                                    @endif
                                                    @if($president->phone)
                                                        <a href="tel:{{ $president->phone }}" class="contact-btn w-8 h-8 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center hover:bg-blue-200" title="Telefon">📞</a>
                                                    @endif
                                                    @if($president->website)
                                                        <a href="{{ $president->website }}" target="_blank" rel="noopener" class="contact-btn w-8 h-8 bg-gray-100 text-gray-700 rounded-full flex items-center justify-center hover:bg-gray-200" title="Web">🌐</a>
                                                    @endif
                                                    @if($president->linkedin)
                                                        <a href="{{ $president->linkedin }}" target="_blank" rel="noopener" class="contact-btn w-8 h-8 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center hover:bg-blue-200" title="LinkedIn">in</a>
                                                    @endif
                                                    @if($president->facebook)
                                                        <a href="{{ $president->facebook }}" target="_blank" rel="noopener" class="contact-btn w-8 h-8 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center hover:bg-blue-200" title="Facebook">f</a>
                                                    @endif
                                                    @if($president->twitter)
                                                        <a href="{{ $president->twitter }}" target="_blank" rel="noopener" class="contact-btn w-8 h-8 bg-black text-white rounded-full flex items-center justify-center hover:bg-gray-800" title="X">x</a>
                                                    @endif
                                                    @if($president->instagram)
                                                        <a href="{{ $president->instagram }}" target="_blank" rel="noopener" class="contact-btn w-8 h-8 bg-pink-100 text-pink-600 rounded-full flex items-center justify-center hover:bg-pink-200" title="Instagram">◎</a>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                            ...
                            @if($otherMembers->count() > 0)
                                <div>
                                    <h2 class="section-title text-2xl font-bold text-gray-900 mb-8 text-center">Yönetim Kurulu Üyeleri</h2>
                                        ...
                                        @foreach($otherMembers as $index => $member)
                                            <div class="member-card bg-white rounded-xl shadow-md overflow-hidden fade-in" style="animation-delay: {{ $index * 0.1 }}s;">
                                                ...
                                                <div class="member-info">
                                                    ...
                                                    <div class="flex flex-wrap justify-center gap-2">
                                                        @if($member->email)
                                                            <a href="mailto:{{ $member->email }}" class="contact-btn w-8 h-8 bg-teal-100 text-teal-600 rounded-full flex items-center justify-center hover:bg-teal-200" title="E-posta">📧</a>
                                                        @endif
                                                        @if($member->phone)
                                                            <a href="tel:{{ $member->phone }}" class="contact-btn w-8 h-8 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center hover:bg-blue-200" title="Telefon">📞</a>
                                                        @endif
                                                        @if($member->website)
                                                            <a href="{{ $member->website }}" target="_blank" rel="noopener" class="contact-btn w-8 h-8 bg-gray-100 text-gray-700 rounded-full flex items-center justify-center hover:bg-gray-200" title="Web">🌐</a>
                                                        @endif
                                                        @if($member->linkedin)
                        ...
                        @else
                            ...
                            <div class="text-center py-16">
                                ...
                                <p class="text-gray-600 mb-8 leading-relaxed">Yönetim kurulu üyeleri yakında eklenecektir.</p>
