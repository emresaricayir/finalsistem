...
@section('title', 'Yeni Galeri Resmi')

@section('content')
<div class="flex justify-between items-center mb-6">
    <h1 class="text-2xl font-bold text-gray-900">Yeni Galeri Resmi</h1>
    <a href="{{ route('admin.gallery-images.index', ['category_id' => $selectedCategory]) }}" class="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 transition-colors">
...
<div class="bg-white rounded-lg shadow p-6">
    <form action="{{ route('admin.gallery-images.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                ...
                <select name="gallery_category_id" id="gallery_category_id" required
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ old('gallery_category_id', $selectedCategory) == $category->id ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="description_tr" id="description_tr" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                          placeholder="Resim açıklaması">{{ old('description_tr') }}</textarea>
            ...
            <div>
                ...
                <textarea name="description_de" id="description_de" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                          placeholder="Bildbeschreibung">{{ old('description_de') }}</textarea>
            ...
            <div class="md:col-span-2">
                ...
                <input type="file" name="image" id="image" accept="image/*" required
                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                <p class="mt-1 text-sm text-gray-500">JPG, PNG, GIF formatında, maksimum 5MB.</p>
                @error('image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="flex justify-end space-x-3 mt-6">
            <a href="{{ route('admin.gallery-images.index', ['category_id' => $selectedCategory]) }}" class="px-4 py-2 text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors">
                İptal
            </a>
...
<script>
// Auto-fill alt text based on title
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            preview.innerHTML = `
                ...
                <p class="text-sm text-gray-500 mt-1">Resim önizlemesi</p>
            `;
