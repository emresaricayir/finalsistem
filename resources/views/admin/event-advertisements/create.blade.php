...
@section('title', 'Yeni Etkinlik Reklamı')
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="bg-gradient-to-r from-purple-600 to-blue-600 px-6 py-4">
                <h3 class="text-2xl font-bold text-white flex items-center">
                    ...
                    Yeni Etkinlik Reklamı
                </h3>
            </div>

            <div class="p-6">
                <form action="{{ route('admin.event-advertisements.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mt-6">
                        ...
                        <textarea class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 @error('content') border-red-500 @enderror"
                                  id="content" name="content" rows="4" placeholder="Reklam açıklamasını yazın...">{{ old('content') }}</textarea>
                        @error('content')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
