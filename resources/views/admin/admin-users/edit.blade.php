...
@section('title', 'Admin Kullanıcısını Düzenle')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-2 text-gray-600">{{ $adminUser->name }} kullanıcısının bilgilerini güncelleyin.</p>
        </div>
    ...
    <div class="bg-white shadow-sm rounded-lg">
        <form method="POST" action="{{ route('admin.admin-users.update', $adminUser) }}" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $adminUser->name) }}" required
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $adminUser->email) }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-2">
                        ...
                        Yeni Şifre <span class="text-gray-500 text-sm">(Boş bırakılırsa değiştirilmez)</span>
                    </label>
                        ...
                        <div class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
                            <label class="flex items-start space-x-3 cursor-pointer">
                                <input type="checkbox" name="roles[]" value="{{ $role->id }}"
                                       {{ in_array($role->id, old('roles', $adminUser->roles->pluck('id')->toArray())) ? 'checked' : '' }}
            ...
            @if($adminUser->roles->count() > 0)
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <h4 class="text-sm font-medium text-blue-900 mb-2">Mevcut Roller:</h4>
