...
<div class="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        ...
        <div class="mb-6">
            ...
            <p class="text-gray-600">{{ $election->title_tr }}@if($election->title_de) / {{ $election->title_de }}@endif</p>
        </div>

        <!-- Form -->
        <div class="bg-white shadow-lg rounded-lg">
            <form action="{{ route('admin.elections.update', $election) }}" method="POST" enctype="multipart/form-data" class="space-y-6 p-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text"
                           name="title_tr"
                           id="title_tr"
                           value="{{ old('title_tr', $election->title_tr) }}"
                ...
                <div>
                    ...
                    <div class="rich-editor-container">
                        ...
                        <div contenteditable="true"
                             ...
                             placeholder="Sayın üyemiz, yazı içeriğinizi buraya yazın...">{!! old('content_tr', $election->content_tr) !!}</div>
                        <textarea name="content_tr" id="content_tr" style="display: none;">{{ old('content_tr', $election->content_tr) }}</textarea>
                    </div>
                    @error('content_tr')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text"
                           name="title_de"
                           id="title_de"
                           value="{{ old('title_de', $election->title_de) }}"
                ...
                <div>
                    ...
                    <div class="rich-editor-container">
                        ...
                        <div contenteditable="true"
                             ...
                             style="min-height: 200px; max-height: 400px; overflow-y: auto;"
                             placeholder="Liebe/r Mitglied, schreiben Sie hier Ihren Text...">{!! old('content_de', $election->content_de) !!}</div>
                        <textarea name="content_de" id="content_de" style="display: none;">{{ old('content_de', $election->content_de) }}</textarea>
                    </div>
                    @error('content_de')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <!-- Mevcut Başkan İmzası -->
                @if($election->president_signature && file_exists(public_path('storage/elections/' . $election->president_signature)))
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        Mevcut Başkan İmzası
                    </label>
                    <div class="flex items-center space-x-4">
                        <img src="{{ asset('storage/elections/' . $election->president_signature) }}"
                             alt="Mevcut Başkan İmzası"
                             class="h-16 w-auto border border-gray-300 rounded">
                        <span class="text-sm text-gray-600">{{ basename($election->president_signature) }}</span>
                    </div>
                </div>
                @endif
                ...
                <div>
                    <label for="president_signature" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ ($election->president_signature && file_exists(public_path('storage/elections/' . $election->president_signature))) ? 'Yeni Başkan İmzası (Mevcut imzayı değiştirmek için)' : 'Başkan İmzası (PNG)' }}
                    </label>
                    <input type="file"
                ...
                <!-- Mevcut Sekreter İmzası -->
                @if($election->secretary_signature && file_exists(public_path('storage/elections/' . $election->secretary_signature)))
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">
                        Mevcut Sekreter İmzası
                    </label>
                    <div class="flex items-center space-x-4">
                        <img src="{{ asset('storage/elections/' . $election->secretary_signature) }}"
                             alt="Mevcut Sekreter İmzası"
                             class="h-16 w-auto border border-gray-300 rounded">
                        <span class="text-sm text-gray-600">{{ basename($election->secretary_signature) }}</span>
                    </div>
                </div>
                @endif
                ...
                <div>
                    <label for="secretary_signature" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ ($election->secretary_signature && file_exists(public_path('storage/elections/' . $election->secretary_signature))) ? 'Yeni Sekreter İmzası (Mevcut imzayı değiştirmek için)' : 'Sekreter İmzası (PNG)' }}
                    </label>
                    <input type="file"
