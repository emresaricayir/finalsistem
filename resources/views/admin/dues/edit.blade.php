...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-2 text-gray-600">{{ $due->member->full_name }} üyesinin aidat bilgilerini güncelleyin.</p>
        </div>
        <div class="mt-4 sm:mt-0">
            <a href="{{ route('admin.dues.show', $due) }}" class="btn-secondary px-6 py-3 rounded-xl font-medium flex items-center">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.dues.update', $due) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="member_id" id="member_id" required
                            ...
                            @foreach($members as $member)
                                <option value="{{ $member->id }}" {{ old('member_id', $due->member_id) == $member->id ? 'selected' : '' }}>
                    ...
                    <div class="bg-blue-50 rounded-xl p-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <p class="font-medium text-gray-900">Seçili Üye</p>
                                <p class="text-sm text-gray-600">{{ $due->member->full_name }}</p>
                                <p class="text-xs text-gray-500">{{ $due->member->email }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-gray-50 rounded-2xl p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <select name="year" id="year" required
                            ...
                            @for($year = 2020; $year <= 2030; $year++)
                                <option value="{{ $year }}" {{ old('year', $due->year) == $year ? 'selected' : '' }}>
                                    {{ $year }}
                                </option>
                            @endfor
                        </select>
                    ...
                    <div>
                        ...
                        <select name="month" id="month" required
                            ...
                            @foreach([
                                ...
                                <option value="{{ $monthNum }}" {{ old('month', $due->month) == $monthNum ? 'selected' : '' }}>
                                    {{ $monthName }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" id="amount" value="{{ old('amount', $due->amount) }}" step="0.01" min="0" required
                ...
                <div class="mt-6">
                    ...
                    <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $due->due_date->format('Y-m-d')) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="p-4 bg-white rounded-xl border border-gray-200">
                            <p class="text-sm text-gray-900">{{ $due->created_at->format('d.m.Y H:i') }}</p>
                            <p class="text-xs text-gray-500">{{ $due->created_at->diffForHumansTr() }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-2xl p-6">
                ...
                <textarea name="notes" id="notes" rows="4"
                          ...
                          placeholder="Aidat hakkında ek notlar...">{{ old('notes', $due->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                <a href="{{ route('admin.dues.show', $due) }}" class="btn-secondary px-8 py-3 rounded-xl font-medium">
